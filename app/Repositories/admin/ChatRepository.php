...
class ChatRepository
{
    ...
    private $serverUrl;    //请求服务地址
    private $format;                //数据格式 json/xml

    function __construct()
    {
    	$this->appKey = config('admin.global.chat.appKey');
    	$this->serverUrl = config('admin.global.chat.serverUrl');
    	$this->appSecret = config('admin.global.chat.appSecret');
    	$this->format = config('admin.global.chat.format');
    }
    ...
    public function curl($action, $params,$contentType='urlencoded') {
        $action = $this->serverUrl.$action.'.'.$this->format;
