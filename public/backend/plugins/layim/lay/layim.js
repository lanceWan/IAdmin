...
var config = {
    ...
    json: function(url,type, data, callback, error){
        return $.ajax({
            type: type,
            url: url,
            data: data,
...
xxim.getGroups = function(param){
    ...
    config.json(config.api.groups,'POST', {}, function(datas){
        if(datas.status === 1){
...
xxim.getDates = function(index){
    ...
    config.json(api[index], 'get',{}, function(datas){
        
        if(datas.status === 1){
            var i = 0, myflen = datas.data.nums, str = '', item;
            if(myflen > 0){
                if(index !== 2){
                    for(; i < myflen; i++){
                        str += '<li data-id="'+ datas.data.id +'" class="xxim_parentnode">'
                            +'<h5><i class="fa fa-caret-right"></i><span class="xxim_parentname">'+ datas.data.name +'</span><em class="xxim_nums">（'+ datas.data.nums +'）</em></h5>'
